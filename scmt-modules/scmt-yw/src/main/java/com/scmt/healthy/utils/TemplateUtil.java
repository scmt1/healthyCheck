...
public class TemplateUtil {
	 ...
	 * @throws Exception 抛出的异常
	 */
	public static  List<String> templateBatchHandData(List <Map<String,Object>> dataList,String Ip,String userId, List<TTemplate> list) throws  Exception{
			...
			try{
				if(map.containsKey("personId")||map.containsKey("wordData")){
					...
					if(map.containsKey("personName")){
						...
						errorPersonName = map.get("personName").toString();
					}
					...
					if(arrMap.size()>0 && arrMap.size() == wordData.size()){
						...
						for (int i = 0; i < arrList.size(); i++) {
							...
							/*if(path!=null && path.trim().length()>0 && path.indexOf("职业体检报告-检查结果及处理意见")>-1){
								urlZy = path;
							}*/
						...
						/*if(Ip.indexOf("首页封面") > -1){//即含首页封面的个人报告,首页模板多占一页要减去1
							pages = pages - 1;
						}*/
						//个检报告 页数为奇数时 加入空白页
				...
				errorDatas.add(errorPersonName + "：" +e.getMessage());
				continue; //将跳过此迭代并跳转到下一个
			}
		}
		//合并后的Word 地址
		if(resListPath.size()>0){
...
//		PdfUtil.MergePdf( resListPath.toArray(new String[resListPath.size()]),PdfPath);
			res.add(PdfPath.substring(PdfPath.indexOf("/tempfile"),PdfPath.length()));
			for(String errorData : errorDatas){
				res.add(errorData);
			}
		}else{
			res.add("");
	...
	public static  List<String> templateBatchHandDataPdf(List <Map<String,Object>> dataList,String Ip,String userId, List<TTemplate> list) throws  Exception{
		...
		for (Map<String,Object> map:dataList) {
			if(map.containsKey("personId")||map.containsKey("wordData")){
				...
				if(map.containsKey("personName")){
					personName = map.get("personName").toString();
					if(map.containsKey("testNum")){
						personName += map.get("testNum").toString();
					}
				}
				ArrayList wordData =  (ArrayList)   map.get("wordData");
