package com.scmt.generator.service.impl;
...
import com.scmt.generator.utils.GenUtil;
import com.scmt.generator.domain.vo.TableInfo;
import com.scmt.generator.repository.ColumnInfoRepository;
...
public class GeneratorServiceImpl implements GeneratorService {
    ...
    public void generator(GenConfig genConfig, List<ColumnInfo> columns) {
        if (genConfig.getId() == null) {
            throw new ScmtException("请先配置生成器");
        }
        try {
            GenUtil.generatorCode(columns, genConfig);
        } catch (IOException e) {
            e.printStackTrace();
            throw new ScmtException("生成失败，请手动处理已生成的文件");
        }
    }

    @Override
    public Result<Object> preview(GenConfig genConfig, List<ColumnInfo> columns) {
        if (genConfig.getId() == null) {
            throw new ScmtException("请先配置生成器");
        }
        List<Map<String, Object>> genList = GenUtil.preview(columns, genConfig);
        return ResultUtil.data(genList);
    }
