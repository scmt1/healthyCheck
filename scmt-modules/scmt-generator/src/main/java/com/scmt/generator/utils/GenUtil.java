...
public class GenUtil {
    ...
    public static void generatorCode(List<ColumnInfo> columnInfos, GenConfig genConfig) throws IOException {
        ...
        // 生成后端代码
        List<String> templates = getAdminTemplateNames();
        GeneratorUtil.genFiles("",genMap.get("className").toString(),genConfig.getPack(),genMap.get("className").toString(),columnInfos,genConfig);
//        for (String templateName : templates) {
//            Template template = engine.getTemplate("generator/admin/"+templateName+".ftl");
...
//        }
        ...
        templates = getFrontTemplateNames();
        GeneratorUtil.genWebFiles("test/"+genConfig.getPath(),genMap.get("className").toString(),columnInfos,genConfig);
//        for (String templateName : templates) {
//
//
//            Template template = engine.getTemplate("generator/front/"+templateName+".ftl");
...
//        }
    ...
    // 获取模版数据
    public static Map<String,Object> getGenMap(List<ColumnInfo> columnInfos, GenConfig genConfig) {
        // 存储模版字段数据
        ...
        // 大写开头的类名
        String className = GeneratorUtil.toCapitalizeCamelCase(genConfig.getTableName());
        // 小写开头的类名
        String changeClassName = GeneratorUtil.toCamelCase(genConfig.getTableName());
        // 判断是否去除表前缀
        if (StringUtils.isNotEmpty(genConfig.getPrefix())) {
            className = GeneratorUtil.toCapitalizeCamelCase(StrUtil.removePrefix(genConfig.getTableName(),genConfig.getPrefix()));
            changeClassName = GeneratorUtil.toCamelCase(StrUtil.removePrefix(genConfig.getTableName(),genConfig.getPrefix()));
        }
        ...
        for (ColumnInfo column : columnInfos) {
            ...
            // 小写开头的字段名
            String changeColumnName = GeneratorUtil.toCamelCase(column.getColumnName().toString());
            // 大写开头的字段名
            String capitalColumnName = GeneratorUtil.toCapitalizeCamelCase(column.getColumnName().toString());
